...
namespace ZTP.Actions
{
    ...
    {
        public static void MovePlayer(bool goLeft, bool goRight, bool goUp, bool goDown, Player player, Canvas canvas)
        {
            if (goLeft == true && Canvas.GetLeft(player.Instance) > 0)
            {
                ...
                player.Direction = 1;
                Canvas.SetLeft(player.Instance, Canvas.GetLeft(player.Instance) - player.Speed);
                
            }
            if (goRight == true && Canvas.GetLeft(player.Instance) + player.Instance.Width < canvas.ActualWidth)
            {
            ...
            if (goUp == true && Canvas.GetTop(player.Instance) > 0)
            {
                ...
                player.Direction = 2;
                Canvas.SetTop(player.Instance, Canvas.GetTop(player.Instance) - player.Speed);
            }
            if (goDown == true && Canvas.GetTop(player.Instance) + player.Instance.Height < canvas.ActualHeight)
            {
        ...
        private static void MovePlayer(Player player, int speed, Canvas canvas)
        {
            ...
            if (player.Direction == 3 && Canvas.GetLeft(player.Instance) + player.Instance.Width < canvas.ActualWidth)
            {
                Canvas.SetLeft(player.Instance, Canvas.GetLeft(player.Instance) + speed);
            }
            ...
            if (player.Direction == 0 && Canvas.GetTop(player.Instance) + player.Instance.Height < canvas.ActualHeight)
            {
                Canvas.SetTop(player.Instance, Canvas.GetTop(player.Instance) + speed);
            }
        }
        ...
        public static void PlayerDash(Player player, int speed, int dashRange, Canvas canvas, ref bool startGifTimer, List<Rectangle> blinkInstances)
        {
            ...
            {
                MovePlayer(player, speed,canvas);
            }
        ...
        public static void EnemyAvoidingOtherEnemy(Monster monster, Rect monsterHitBox, Rect otherMonsterHitBox, Canvas canvas)
        {
            if(!(Canvas.GetLeft(monster.Instance) > 0 || Canvas.GetLeft(monster.Instance) + monster.Instance.Width < canvas.ActualWidth || Canvas.GetTop(monster.Instance) > 0 || Canvas.GetTop(monster.Instance) + monster.Instance.Height < canvas.ActualHeight))
            {
                return;
            }
